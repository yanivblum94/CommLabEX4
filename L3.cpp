...
int L3::sendToL3(byte *sendData, size_t sendDataLen, std::string srcIP, std::string destIP, bool debug) {
	...
	int res = lowerInterface->sendToL2(buff, (uint16_t)(IP_HEADER_SIZE + packet->DataLayer4Len), debug);
	delete packet;
	delete[] buff;
	return res;
}
...
int L3::recvFromL3(byte *recvData, size_t recvDataLen, bool debug) {
	...
	if (!(packet->ConverPacketFromBinary(frame, data_size - ETHERNET_HEADER_SIZE)) || !(packet->IsPacketValid())) {
		delete packet;
		delete[] packet_buff;
		return FAIL_CODE;
	}
	...
	packet->PrintPacket();
	memcpy(recvData, packet->DataLayer4, packet->DataLayer4Len); //copy packet l4
	int res = packet->DataLayer4Len;
	delete packet;
	delete[] packet_buff;
	return res;
}
